...
- PWA/Web microphone capture must not stay in listening mode forever. One microphone press captures one question, uses non-continuous recognition, actively stops after final speech or stable interim speech, and releases the mic button on no-speech/error/timeout.
...
- PWA/Web teacher must never display a half sentence from Gemini. Validate backend answers before displaying or speaking them; if the answer looks incomplete, show a Gemini problem message instead of pretending with local fallback text.
- PWA/Web must not pretend local fallback text is a real Gemini teacher answer. If the Gemini backend is unavailable, rate-limited, returns a half answer, or repeats a rejected example, show a clear Gemini problem message; do not increment the 5-question count for that failed attempt.
